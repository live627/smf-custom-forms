...
function CustomForm()
{
	...
	if (isset($_REQUEST['thankyou']))
	{
		...
		)
		{
			...
			if (isset($_GET['submit']))
			{
				...
				if (!$fail_submit)
				{
					...
					if (isset($_FILES['attachment']['name']))
					{
						...
						isAllowedTo('post_attachment', $board);

						$quantity = 0;
						$total_size = 0;
						$attachIDs = array();
						foreach (diverse_array($_FILES['attachment']) as $attachment)
						{
							if ($attachment['name'] == '')
								continue;
							...
							if (!empty($modSettings['attachmentNumPerPostLimit']) && $quantity > $modSettings['attachmentNumPerPostLimit'])
							{
								...
								fatal_lang_error(
									...
									false,
									array($modSettings['attachmentNumPerPostLimit'])
								);
							}
							...
							else
							{
								...
								if (in_array('bad_extension', $attachmentOptions['errors']))
								{
									...
									fatal_error(
										$attachmentOptions['name'] . '.<br />' . $txt['cant_upload_type'] . ' ' . $modSettings['attachmentExtensions'] . '.',
										false
									);
								}
			...
			$context['num_allowed_attachments'] =
				empty($modSettings['attachmentNumPerPostLimit']) ? 50 : $modSettings['attachmentNumPerPostLimit'];
			$context['can_post_attachment'] =
				!empty($modSettings['attachmentEnable']) && $modSettings['attachmentEnable'] == 1 && allowedTo(
					'post_attachment'
				) && $context['num_allowed_attachments'] > 0;
			$context['allowed_extensions'] = strtr($modSettings['attachmentExtensions'], array(',' => ', '));
			$context['attachment_restrictions'] = array();
			$attachmentRestrictionTypes =
				array('attachmentNumPerPostLimit', 'attachmentPostLimit', 'attachmentSizeLimit');
			foreach ($attachmentRestrictionTypes as $type)
				if (!empty($modSettings[$type]))
					$context['attachment_restrictions'][] =
						sprintf($txt['attach_restrict_' . $type], $modSettings[$type]);
